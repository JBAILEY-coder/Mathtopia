...
function Register(e) {
  ...
  document.getElementById("content2").classList.remove("hidden") //task 3
}
...
function showall() {
  ...
  for (var i = 0; i < PlayersData.length; i++) {
    document.getElementById("showallplayers").innerHTML += `
    Player's Name = ${PlayersData[i].fname} ${PlayersData[i].lname} 
		Age = ${PlayersData[PlayersData.length - 1].age} `

    for (var t = 0; t < PlayersData[i].questions.length; t++) {
      document.getElementById("showallplayers").innerHTML += `
			Question = ${PlayersData[i].questions[t]}
			${PlayersData[i].fname}'s Answer = ${PlayersData[i].user_answer[t]}
			Status of the Answer = ${PlayersData[i].vali[t]}`
    }
    document.getElementById("showallplayers").innerHTML += `
    Percentage Score = ${percent2[i]}`
  }
}
...
function count() {
  ...
  for (var i = 0; i < PlayersData.length; i++) {
    ...
    if (percent2[i] >= 50.0 && percent2[i] <= 59.0) {
      bet50 += 1
    }
    if (percent2[i] >= 60.0 && percent2[i] <= 69.0) {
      bet60 += 1
    }
    if (percent2[i] >= 70.0 && percent2[i] <= 79.0) {
      bet70 += 1
    }
    if (percent2[i] >= 80.0 && percent2[i] <= 89.0) {
      bet80 += 1
    }
    if (percent2[i] >= 90.0 && percent2[i] <= 99.0) {
      bet90 += 1
    }
    if (percent2[i] == 100) {
      at100 += 1
